...
export function MapaProspectos({
  ...
  useEffect(() => {
    ...
    const mapa = L.map(contenedor.current, { scrollWheelZoom: false }).setView([4.61, -74.08], 12);
    ...
    contenedor.current.addEventListener('click', alClic);

    return () => {
      contenedor.current?.removeEventListener('click', alClic);
      mapa.remove();
  ...
  // Redibujar marcadores cuando cambian los datos.
  useEffect(() => {
