...
export async function cargarBaseInicial(): Promise<BaseInicial> {
  // El archivo pesa ~3 MB y el servidor no lo comprime: en datos móviles
  // puede tardar más de un minuto. Tiempo amplio y error que explica qué pasó.
  let respuesta: Response;
  try {
    respuesta = await fetchConTimeout('./empresas-bogota.json', { cache: 'no-cache' }, 180000);
  } catch {
    throw new Error(
      'La base de empresas está tardando mucho en descargar (pesa ~3 MB). Con wifi va más rápido; intenta de nuevo.',
    );
  }
  if (!respuesta.ok) {
